...
**통합 출처**: PRD-001 (기본 워크트리 세팅), PRD-002 (Compose 조합 시스템), PRD-003 (리소스 가져오기 다중 선택)
...
**방식 B: 기존 파일 등록 (다중 선택 지원)**
1. 파일 탐색기로 로컬 파일/디렉토리 선택 (**여러 항목 동시 선택 가능**)
2. 선택한 각 항목의 내용을 앱 데이터로 복사하여 등록
   - 스킬: 디렉토리 단위 등록 (SKILL.md + 하위 파일들) — **여러 폴더 동시 선택 가능**
   - 에이전트/커맨드: .md 파일 등록 — **여러 파일 동시 선택 가능**
   - MCP: .json 파일 등록 — **여러 파일 동시 선택 가능**
3. 각 항목별 유효성 검증 후 등록 완료 (일부 실패 시 나머지는 계속 처리)
4. 결과 표시: 성공/실패 건수 알림
...
- **성능**: 조합 적용 시 .claude/ 구조 생성은 1초 이내. 등록된 리소스가 많아도 목록 스크롤이 매끄러워야 함. 다중 가져오기 시에도 UI 블로킹 없이 비동기 처리
...
| 다중 가져오기 시 일부 항목이 유효하지 않음 (스킬: SKILL.md 미존재, 파일: 파싱 실패 등) | 해당 항목 스킵, 나머지 정상 등록 + 실패 건수 표시 |
| 다중 가져오기 시 선택한 항목이 전부 실패 | 에러 메시지로 전체 실패 사유 표시 |
...
| 2026-02-27 | 리소스 가져오기 다중 선택 지원: 스킬 폴더/파일 여러 개 동시 선택 및 일괄 등록, 에러 격리 | PRD-003 | Claude & godaehyeon |
